...
- `macos/`：Swift Package，`WhatShotCore`（模型、网络、持久化、同步）+ `WhatShotApp`（SwiftUI 菜单栏应用）+ Core 测试
...
- 常驻空闲内存 < 60MB：分页懒加载、不一次性载入全量数据、海报内存用 NSCache
...
- 站点域名必须可配置（该站域名经常更换，站点自身维护多个备用域名）；接口解析必须容错，字段缺失不报错；`ejs`(更新至X集/全集/空)、`episodes`(总集数, "0"=未知)、`seed_num`/`wp_num`、`doub_id`/`IMDB_number` 等字段语义见 docs/requirements.md
