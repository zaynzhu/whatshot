...
> [!TIP]
> WhatShot 是一款**纯本地**的 macOS 菜单应用，专注两个问题：某部剧/电影现在**热不热**（热门榜、资源数、评分），以及某部剧**更新到第几集**（更新至 X 集 / 共 Y 集 / 全集完结）。无后端、无账号、数据不出本机。

## ✨ Features

- **热门榜三窗口** -- 近日 / 本周 / 本月热门影视榜，榜首以 hero 大卡呈现，一眼看到当下最热
- **播出进度追踪** -- 「更新至 9 集」「共 24 集」「全集完结」独立信息行，进度线可视化追剧进度
- **剧集 / 电影双分类画廊** -- 海报墙式浏览最近更新的剧集与电影，窗口自适应布局
- **豆瓣 / IMDb 双评分 + 资源数** -- 每张卡片聚合站内热度信号，等宽数字排版
- **低资源常驻** -- 空闲内存约 15MB，SQLite 单文件存储，海报缓存上限可调，平时零定时器空转
- **低频自动同步** -- 默认 6 小时一次短促同步（每轮约 10~20 个请求），可手动触发、可关闭
- **数据源韧性** -- 内置 12 个官方域名池自动探活择优、失败秒级降级换路由、接口字段容错解析、失效海报占位图自动拦截
...
```bash
./scripts/test-macos.sh   # Swift Testing，13 个测试
```
...
启动应用默认进入「热门榜」标签：近日 / 本周 / 本月三个榜单切换，窗口足够宽时榜首以 hero 大卡展示（名次、标题、播出进度、评分、简介与进度线），其余条目按画廊网格排布。

**追剧集更新**

切到「剧集」标签，按站内更新时间排序浏览。每张卡片显示片名、更新至 X 集（琥珀色进行中信号）、豆瓣 / IMDb 评分与资源数；滚动到底部自动懒加载更多。

**调整同步与缓存**

「设置」标签可自定义数据源域名（可选，最高优先；留空则自动使用内置域名池——同步前探活选当前最快的官方路由，故障自动切换），同步频率与海报缓存上限（默认 300MB，可调低或关闭），并显示当前路由与实测延迟、磁盘占用。
