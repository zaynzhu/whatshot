...
- **剧集 / 电影双分类画廊** -- 海报墙式浏览最近更新的剧集与电影，窗口自适应布局；⌘F 本地搜索片名 / 原名 / 别名，与筛选排序组合（只搜已同步内容，不做外部标题搜索）
- **首播时间线** -- 剧集按首播日期倒序排列（老剧重供不再冒头），配套年代/状态/类型/地区本地筛选与详情浮层（简介、演职员、集数推进记录、豆瓣/IMDb 外链）。**首播日双源补全**：豆瓣按已有 douban_id 直连补全（无需 Key）；豆瓣被限流时，另设 TMDB 兜底补有 IMDb 的欧美剧集该季首播日（不覆盖豆瓣已写日期，可选，需在设置页填 TMDB Bearer Token）
- **海报兜底与 S3 镜像** -- 站方图床失效（明文 http 被 macOS ATS 拦截）的条目，自动从豆瓣/TMDB 补图；配置自有 S3 兼容存储（RustFS/MinIO）后，兜底取到的图先镜像进你的桶再展示，不再依赖外站图床（设置页填端点/桶名/密钥，局域网端点走 macOS 本地网络豁免）
- **追剧与更新汇总** -- 详情浮层一键关注，「追剧」标签页集中展示最新集数、完结状态与检查时间；作品离开榜单后每轮同步仍有限检查（每轮上限 20 条、最久未检查优先），自上次查看的集数变化（如"更新至9集 → 更新至12集"）汇总在页首，标签圆点提示未读、进入即读
- **豆瓣 / IMDb 双评分** -- 每张卡片聚合评分信号，等宽数字排版
- **外部热度（可选接入 WhatsNew）** -- 配置你自有的 WhatsNew 服务（NAS/局域网）后，外部热度标签页以海报墙浏览其来源榜单（Netflix 周榜、优酷/爱奇艺/腾讯预约、bilibili 榜等），单选一个来源榜查看、名次角标与涨跌标记；能精确匹配上本地库的条目可点进详情并追剧，豆瓣作品在详情里追加 WhatsNew 豆瓣评分对照（含投票数与采集时间）。显式启用 + 填入服务地址才发请求，关闭或未配置零请求；**追剧作品每轮同步自动反查 WhatsNew**（不在来源榜单内也能拿到真实名次与豆瓣评分对照；服务未部署新版时静默降级，部署后自动生效）；榜单语义如实标注（豆瓣 TOP250 口碑 / 待播顺序 / 预约不显示涨跌），不做跨源混算
- **低资源常驻** -- 空闲内存约 15MB，SQLite 单文件存储，海报缓存上限可调，平时零定时器空转
- **低频自动同步** -- 默认 6 小时一次短促同步（每轮约 10~20 个请求），可手动触发、可关闭
- **同步状态透明** -- 顶栏状态区分"已同步 / 部分完成 / 出错 / 已停止"并随时间演进；同步中实时显示当前阶段与补全计数，可随时手动停止（已提交数据保留，未完成补全下轮自动继续）；点击状态展开分步详情（各榜单与补全步骤逐项结果、错误原文与重试说明），豆瓣限流等小故障不再伪装成"出错"
...
```bash
./scripts/test-macos.sh   # Swift Testing 测试套件
```
...
切到「剧集」标签，默认按**首播日期倒序**浏览（可切回资源更新排序）。卡片显示片名、更新至 X 集（琥珀色进行中信号）、首播日期与豆瓣 / IMDb 评分；页头筛选条支持年代 / 播出状态（播出中 / 已完结） / 类型 / 地区四维筛选；⌘F 唤起本地搜索（片名 / 原名 / 别名，与筛选组合）；点击卡片弹出详情（简介、演职员、集数推进记录、豆瓣 / IMDb 外链）；滚动到底部自动懒加载更多。

**关注追剧与更新汇总**

在详情浮层点「追剧」关注作品，「追剧」标签页集中展示关注列表的最新集数、完结状态与最近检查时间；离开热门榜与最近更新页的作品每轮同步仍会自动检查更新。自上次查看以来有集数变化的作品汇总在页首（"更新至9集 → 更新至12集 · 本地发现时间"），标签圆点提示未读，进入页面即标记已读。
...
如果你在 NAS 上自部署了 [WhatsNew](https://github.com/zaynzhu/whatsnew)，「设置 → 外部热度（WhatsNew）」打开开关并填入服务地址（如 `http://<NAS-IP>:19993`）后，「外部热度」标签页以海报墙浏览其来源榜单：顶部来源标签单选一个榜（不同榜单名次不可比，不提供混排的"全部"视图），卡片段落显示名次、涨跌与站方采集时间；能精确匹配上本地库的作品标"已关联"，点击进入本地详情并可追剧，豆瓣作品详情里会追加 WhatsNew 豆瓣评分对照。关闭开关或未配置地址时本功能不发任何请求。
...
| [docs/lessons/INDEX.md](docs/lessons/INDEX.md) | 经验库：已验证的坑与现成方案（排坑、调试前先查） |
