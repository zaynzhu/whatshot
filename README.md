...
- **外部热度（可选接入 WhatsNew）** -- 配置你自有的 WhatsNew 服务（NAS/局域网）后，外部热度标签页以海报墙浏览其来源榜单（Netflix 周榜、优酷/爱奇艺/腾讯预约、bilibili 榜等），单选一个来源榜查看、名次角标与涨跌标记；能精确匹配上本地库的条目可点进详情并追剧，豆瓣作品在详情里追加 WhatsNew 豆瓣评分对照（含投票数与采集时间）。显式启用 + 填入服务地址才发请求，关闭或未配置零请求；榜单语义如实标注（豆瓣 TOP250 口碑 / 待播顺序 / 预约不显示涨跌），不做跨源混算
